...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  uint8_t num = 0U;
  uint8_t interface;
  uint8_t misc_interface;

  interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, HID_BOOT_CODE, 0xFFU);
  misc_interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, HID_JOYSTICK_NON_BOOT_CODE, 0xFFU);
  ...
  else if(phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol  == HID_JOYSTICK_NON_BOOT_CODE)
  {
...
//                                HID_Handle->InPipe);
    ...
    case HID_SEND_DATA:
    			...
    			break;
	case HID_BUSY:
		if((USBH_LL_GetURBState(phost, HID_Handle->OutPipe) == USBH_URB_DONE)||(USBH_LL_GetURBState(phost, HID_Handle->OutPipe) == USBH_URB_IDLE))
			{
			HID_Handle->state = HID_SYNC;
			}
...
HID_TypeTypeDef USBH_HID_GetDeviceType(USBH_HandleTypeDef *phost)
{
  ...
  if (phost->gState == HOST_CLASS)
  {
    ...
    else
    {
      ...
      {
		if (InterfaceProtocol == HID_JOYSTICK_NON_BOOT_CODE)
		{
			type = HID_JOYSTICK;
		}
      }
...
__weak void USBH_HID_EventCallback(USBH_HandleTypeDef *phost)
{
  ...
  //UNUSED(phost);
	...
	HID_HandleTypeDef *HID_Handle = (HID_HandleTypeDef *) phost->pActiveClass->pData;
	HID_Handle->state = HID_IDLE;
	if(USBH_HID_GetDeviceType(phost) == HID_JOYSTICK)
		{
		...
		DJoyStick.Hat=((DJoyStick.RAW_IN[0]>>20)&0x0000000F);
		for(i=0;i<8;i++)
