...
void SW_I2C_Read_Data_Mag(uint8_t sel, uint8_t *data)
{
    int      x;
    uint8_t  readdata = 0;
    uint8_t i = 0;
...
//        data[0] <<= 1;
//        data[1] <<= 1;
//        if (SW_I2C_ReadVal_SDA(1))
//            data[0] |= 0x01;
//        if (SW_I2C_ReadVal_SDA(2))
//            data[1] |= 0x01;
        for(i = 0; i < QMC5883L_AMOUNT; i++)
        {
        	data[i] <<=1;
        	if (SW_I2C_ReadVal_SDA(i+1))
        		data[i] |= 0x01;
        }
...
//    pdata[rcnt-1] = SW_I2C_Read_Data(sel);
	for ( index = 0 ; index < rcnt ; index++){
		TIMER__Wait_us(SW_I2C_WAIT_TIME);
		SW_I2C_Read_Data_Mag(sel,pdata + index*QMC5883L_AMOUNT);
	}

	SW_I2C_Read_Data_Mag(sel,pdata + (rcnt - 1)*QMC5883L_AMOUNT);
